import { AuthenticatedRole, ParsePermission, ParseCode, parseListPermissions, PermissionFormat, Article, Validate, FilePermission } from '../src/note-gate';
...
describe("Parse client user's role permissions", () => {
    ...
    test("To Permission", done => {
        ...
        User.can = function( Action : any ) :boolean { return Validate(Action, this.role) }

        User.role = AuthenticatedRole.ArticleManager

        const ManagementArticles : PermissionFormat = { 'file.write': FilePermission.Write }
